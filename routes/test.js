...
const {myFirebase,myFirestore} = require('../config/firebaseNormal')
    ...
    myFirestore.collection('users')
        ...
        then(function (querySnapshot) {
            querySnapshot.forEach(function (doc) {
                myFirestore.collection('users').doc(doc.id).update({contacts:firebase.firestore.FieldValue.arrayUnion(req.params.id)})
            })
            }
        )
    myFirestore.collection('users')
        ...
        then(function (querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    myFirestore.collection('users').doc(doc.id).update({contacts:firebase.firestore.FieldValue.arrayUnion(String(req.user._id))})
                })
            }
        )
